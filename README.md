# 简易AI对话(微信 AI 分身 · iLink 官方 API 版)
...
├── config/             # 配置文件(含密钥,勿提交;首次运行自动生成)
├── tools/              # 工具目录(白名单安全工具,AI 按需调用)
├── data/               # history/(L1) + vector_store/(L2) + emotion/(情绪态) + state.json(勿提交)
├── logs/               # 运行日志(只记报错,不删旧日志)
├── txt/                # 设计文档(DESIGN / MEMORY_DESIGN / REDESIGN / UPGRADE_DESIGN / PROJECT_GUIDE 等)
└── docs/               # iLink 协议参考
```
...
首次运行会自动生成默认配置文件;也可参考 `config.py` 中的 `DEFAULT_SYSTEM` 默认值:
...
// config/系统配置文件.json
{
  ...
  "analysis_model_information": {      // 分析模型(情绪分析/压缩/搜索决策,网页可改)
    "name": "deepseek-v4-flash", "apikey": "<DEEPSEEK_API_KEY>",
    "base_url": "https://api.deepseek.com", "temperature": 0.1, "max_tokens": 4096
  },
  "memory_information": {              // 记忆设置
    "enabled": true, "l1_context_turns": 10, "l1_summary_rounds": 10,
    "l1_maxlen": 10, "l2_top_k": 5,
    "dedup_threshold": 0.85, "retrieval_sim_weight": 0.7,
    "retrieval_min_similarity": 0.45, "importance_half_life_days": 14,
...
// config/用户画像.json       —— 你的画像(称呼/关系/爱好/性格/长期事实等)
```

> ⚠️ **`config/` 与 `data/` 已被 `.gitignore` 忽略,请勿提交**:`config/系统配置文件.json` 含 API 密钥,`data/state.json` 含微信 context_token,泄露会造成财产/信息风险。
